...
class DDConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"x_q_lt shape")
        x_q_lt = self._get_x_q(x, q_lt.to(device_cuda), N)
        # print(f"x_q_rb shape")
        x_q_rb = self._get_x_q(x, q_rb.to(device_cuda), N)
        # print(f"x_q_lb shape")
        x_q_lb = self._get_x_q(x, q_lb.to(device_cuda), N)
        # print(f"x_q_rt shape")
        x_q_rt = self._get_x_q(x, q_rt.to(device_cuda), N)
        # assert g_lt.shape == x_q_lt.shape[1:], f"Shape mismatch: g_lt {g_lt.shape}, x_q_lt {x_q_lt.shape[2:]}"
        # assert g_rb.shape == x_q_rb.shape[1:], f"Shape mismatch: g_rb {g_rb.shape}, x_q_rb {x_q_rb.shape[2:]}"
        # assert g_lb.shape == x_q_lb.shape[1:], f"Shape mismatch: g_lb {g_lb.shape}, x_q_lb {x_q_lb.shape[2:]}"
        # assert g_rt.shape == x_q_rt.shape[1:], f"Shape mismatch: g_rt {g_rt.shape}, x_q_rt {x_q_rt.shape[2:]}"
...
# x_q_rb = self._get_x_q(x, q_rb.to(device_cuda), N)
    def _get_x_q(self, x, q, N):
        # print(f"x shape: {x.shape}, q shape: {q.shape}", N)
        b, h, w, _ = q.size()
        padded_w = x.size(2)
        c = x.size(1)
        ...
        # print(f"index shape: {index.shape}","x.shape",x.shape, "b", b, "c", c, "h", h, "w", w, "N", N)
