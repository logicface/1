...
class myToTensor:
    ...
    def __call__(self, data):
        image, mask = data
        # 如果 image 或 mask 是二维数组，添加一个通道维度
        if len(image.shape) == 2:
            image = image.unsqueeze(2)  # 将形状从 [H, W] 转换为 [H, W, 1]
        if len(mask.shape) == 2:
            mask = mask.unsqueeze(2)  # 将形状从 [H, W] 转换为 [H, W, 1]
        # 然后再进行 permute
...
class myResize:
    ...
    def __call__(self, data):
        image, mask = data
        if len(image.shape) == 2:  # 如果是二维图像
            image = image.unsqueeze(0)  # 将其变成 [1, H, W]
        if len(mask.shape) == 2:  # 如果是二维掩码
            mask = mask.unsqueeze(0)  # 将其变成 [1, H, W]

        image_resized = TF.resize(image, [self.size_h, self.size_w])
        mask_resized = TF.resize(mask, [self.size_h, self.size_w])
...
class myNormalize:
    ...
    def __call__(self, data):
        img, msk = data
        # 如果 img 是 PIL 图像对象，先转换为 numpy 数组
        if isinstance(img, Image.Image):
            img = np.array(img)
        # 如果 img 是 numpy 数组，转换为 torch 张量
        if isinstance(img, np.ndarray):
            img = torch.tensor(img, dtype=torch.float32)
        # 打印调试信息，查看 img 和其他变量的类型
        # print(type(self.mean), type(img), type(self.std), self.std)
        # 进行标准化操作
